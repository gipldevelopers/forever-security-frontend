...
// Gallery methods
async getGallery(queryParams = '') {
    try {
        return await this.request(`api/gallery${queryParams}`);
    } catch (error) {
        console.error('❌ Paginated gallery failed, trying non-paginated:', error);
        return await this.request('api/gallery/all');
    }
}

async getAllGallery() {
    return this.request('api/gallery/all');
}
...
// Note: uploadImage method should handle FormData differently
// We'll handle this directly in the component
...
async toggleImageStatus(imageId) {
    return this.request(`api/gallery/${imageId}/toggle`, {
        method: 'PATCH',
    });
}

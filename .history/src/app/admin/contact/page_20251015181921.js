...
export default function AdminContactPage() {
  ...
  const fetchSubmissions = async () => {
    try {
      ...
      if (response && response.success) {
        ...
        if (errorMsg.includes('token') || errorMsg.includes('auth') || errorMsg.includes('401')) {
          setError('Authentication required. Please login again.');
        }
      }
    } catch (error) {
      console.error('💥 Fetch contact submissions error:', error);
      setError('Failed to load contact submissions. Please check if you are logged in.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleDelete = async (submissionId) => {
    ...
    try {
      ...
      console.log(`🗑️ Deleting submission with ID: ${submissionId}`);
      
      const response = await apiService.deleteContact(submissionId);
      ...
      if (response && response.success) {
        ...
        // Close modal if the deleted submission was open
                  ...
                  <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2 lg:flex-col lg:items-end lg:gap-3">
                      ...
                      <button
                        ...
                        className="inline-flex items-center gap-2 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors text-sm disabled:opacity-50 disabled:cursor-not-allowed"
                      >
